...
// Середов А.С. 
// Вариант 9 - Реализовать обратный нечеткий логический вывод на основе операции нечеткой композиции (min({1}∪{max({0}∪{x_i+y_i-1}) | i}})
// 24.12.2022
// Использованные материалы:
// Код студентов гр 921701 Пилипейко В.И. и Драгун В.А. (взят за основу)
...
//подаем матрицу и входящий сетт
...
vector<vector<vector<pair<float, float>>>> compute_reverse_composition(arr array, impl_matrix matrix) {
	...
	for (size_t j = 0; j < matrix[0].size(); j++)
	{
		...
		for (size_t k = 0; k < matrix.size(); k++)
		{
			...
			//(min({1}∪{max({0}∪{x_i+y_i-1}) | i}})
			for (int t = 0; t < matrix.size(); t++) {
				if (t == k) {
					solution.push_back({ lower, higher });
				}
				else {
					float max = 0 <= (array[j].second + matrix[t][j].second - 1) ? array[j].second : 0;
					solution.push_back({ 1 , max });
				}
			}
